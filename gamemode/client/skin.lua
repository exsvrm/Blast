...
TriggerEvent("zh3b35f:modelLoaded")
end)
end
...
AddEventHandler("zh3b35f:loadDefaultModel", function(loadMale, cb)
LoadDefaultModel(loadMale, cb)
end)

AddEventHandler("zh3b35f:getData", function(cb)
...
AddEventHandler("zh3b35f:change", function(key, val)
Character[key] = val

if key == "sex" then
TriggerEvent("zh3b35f:loadSkin", Character)
else
ApplySkin(Character)
end
end)

AddEventHandler("zh3b35f:getSkin", function(cb)
cb(Character)
end)

AddEventHandler("zh3b35f:modelLoaded", function()
...
RegisterNetEvent("zh3b35f:loadSkin")
AddEventHandler("zh3b35f:loadSkin", function(skin, cb)
if skin["sex"] ~= LastSex then
LoadSkin = skin

if skin["sex"] == 0 then
TriggerEvent("zh3b35f:loadDefaultModel", true, cb)
else
TriggerEvent("zh3b35f:loadDefaultModel", false, cb)
end
else
ApplySkin(skin)
...
RegisterNetEvent("zh3b35f:loadClothes")
AddEventHandler("zh3b35f:loadClothes", function(playerSkin, clothesSkin)
...
TriggerEvent("zh3b35f:loadDefaultModel", true)
else
TriggerEvent("zh3b35f:loadDefaultModel", false)
end
else
...
RegisterCommand("getSkin", function(...)
    ...
    TriggerEvent("zh3b35f:change", "pants_1", 14)
    TriggerEvent("zh3b35f:change", "pants_2", 1)
    TriggerEvent("zh3b35f:change", "skin", 3)
    TriggerEvent("zh3b35f:change", "torso_1", 91)
    TriggerEvent("zh3b35f:change", "arms", 51)
    TriggerEvent("zh3b35f:change", "tshirt_1", 15)
    TriggerEvent("zh3b35f:change", "shoes_1", 34)
    TriggerEvent("zh3b35f:change", "mask_1", 115)
    TriggerEvent("zh3b35f:change", "mask_2", 14)
    TriggerEvent("zh3b35f:change", "bags_1", 31)
    TriggerEvent("zh3b35f:change", "watches_1", 20)
end)

clothes = {
    Base = { Title = "Clothes" },
    Data = { currentMenu = "clothes", TypeBlue = true },
    Events = {

    onSelected = function(self, _, btn, MP, menuData, currentButton, currentSlt, result)
        -- if btn.action == "top" then
        --     SetPedComponentVariation(PlayerPedId(), 11, btn.iterator)
        -- end
    ...
    onButtonSelected = function(currentMenu, currentBtn, menuData, newButtons, self)
        ...
        --     SetPedComponentVariation(PlayerPedId(), 11, currentBtn)
        -- end
    end,

    onAdvSlide = function(self, _, btn, currentBtn, currentButtons)
        if btn.action == "top" then
            SetPedComponentVariation(PlayerPedId(), 11, GetPedDrawableVariation(PlayerPedId(), 11), btn.advSlider[3])
        end
    end,
},
Menu = {
    ...
    ["clothes"] = {
        ...
        extra = true,
        slide = function(_, b)
            SetPedComponentVariation(PlayerPedId(), b.varType, b.slidenum)
            b.advSlider = {0, GetNumberOfPedPropTextureVariations(PlayerPedId(), 1, b.slidenum) -1, 0, 0}
        end,
        buttons = function()
          return {
            { ID = 1, varType = 11, name = "Variation", slidemax = 382, advSlider = {0, GetNumberOfPedTextureVariations(PlayerPedId(), 11), 0}, action = "top" }
          }
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(0, keys["F10"]) then
            SetPedComponentVariation(PlayerPedId(), 8, 15)
            SetPedComponentVariation(PlayerPedId(), 3, 62)
            CreateMenu(clothes)
        end
    end
end)